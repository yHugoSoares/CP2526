...
\subsubsection*{1. Catamorfismo de níveis (\ensuremath{\Varid{levels}})}
...
\textbf{Estratégia}: Utilizamos um catamorfismo de árvores binárias, definido como \ensuremath{\Varid{levels}\mathrel{=}\llparenthesis\, \Varid{glevels}\,\rrparenthesis}. Um catamorfismo consome a estrutura recursiva da árvore, substituindo cada construtor (\ensuremath{\Conid{Empty}} e \ensuremath{\Conid{Node}}) por operações definidas no gene.

\textbf{Definição do Gene \ensuremath{\Varid{glevels}}}:

O gene processa os dois casos do tipo \ensuremath{\Conid{BTree}\;\Varid{a}}:

\begin{itemize} 

\item \textbf{Caso base} (\ensuremath{\Conid{Empty}}): Uma árvore vazia não tem níveis, logo retornamos a lista vazia \ensuremath{[\mskip1.5mu \mskip1.5mu]}.

\item \textbf{Caso recursivo} (\ensuremath{\Conid{Node}\;(\Varid{a},(\Varid{l},\Varid{r}))}): Recebemos o valor da raiz \ensuremath{\Varid{a}} e as listas de níveis já computadas para as subárvores esquerda \ensuremath{\Varid{l}} e direita \ensuremath{\Varid{r}}. A estratégia é:
  \begin{enumerate}
    \item Criar o nível 0 contendo apenas a raiz: \ensuremath{[\mskip1.5mu \Varid{a}\mskip1.5mu]}
    \item Combinar os níveis restantes de ambas as subárvores usando \ensuremath{\Varid{zipWithPlus}}
  \end{enumerate}

\end{itemize}

\textbf{Função auxiliar \ensuremath{\Varid{zipWithPlus}}}: Esta função funde duas listas de listas, concatenando elemento a elemento. Lida com três casos:
\begin{itemize}
  \item Se a lista esquerda está vazia, retorna a direita
  \item Se a lista direita está vazia, retorna a esquerda  
  \item Se ambas têm elementos, concatena as cabeças \ensuremath{(\Varid{x}\mathbin{+\!\!+}\Varid{y})} e aplica recursivamente às caudas
\end{itemize}
...
\item \ensuremath{[\mskip1.5mu \Varid{a}\mskip1.5mu]} cria o primeiro nível (a raiz).
\item zipWith (++) junta o nível n da esquerda com o nível n da direita para formar o novo nível n+1.

\end{itemize}

\subsubsection*{2. Anamorfismo de travessia (\ensuremath{\Varid{bft}})}
...
\textbf{Estratégia}: Implementamos \ensuremath{\Varid{bft}} como um anamorfismo de listas, seguindo a abordagem de Okasaki baseada numa fila (\emph{queue}). Um anamorfismo \emph{gera} uma estrutura (neste caso, uma lista) a partir de um estado inicial.

\textbf{Estado}: O estado é uma fila de árvores \ensuremath{[\mskip1.5mu \Conid{BTree}\;\Varid{a}\mskip1.5mu]} que representa as árvores ainda por processar. Inicialmente, a fila contém apenas a árvore original: \ensuremath{[\mskip1.5mu \Varid{t}\mskip1.5mu]}.

\textbf{Definição do Gene \ensuremath{\Varid{geneBFT}}}:

O gene determina o próximo elemento da lista de saída e o novo estado:

\begin{itemize} 

\item \textbf{Caso 1} (fila vazia \ensuremath{[\mskip1.5mu \mskip1.5mu]}): A travessia terminou. Retornamos \ensuremath{i_1\;()} para sinalizar o fim da lista.

\item \textbf{Caso 2} (primeiro elemento é \ensuremath{\Conid{Empty}}): Árvores vazias não contribuem com elementos. Ignoramos e processamos recursivamente o resto da fila.

\item \textbf{Caso 3} (primeiro elemento é \ensuremath{\Conid{Node}\;(\Varid{a},(\Varid{l},\Varid{r}))}): 
  \begin{enumerate}
    \item Extraímos o valor da raiz \ensuremath{\Varid{a}} como próximo elemento da lista
    \item Adicionamos as subárvores \ensuremath{\Varid{l}} e \ensuremath{\Varid{r}} ao \textbf{fim} da fila: \ensuremath{\Varid{ts}\mathbin{+\!\!+}[\mskip1.5mu \Varid{l},\Varid{r}\mskip1.5mu]}
    \item Retornamos \ensuremath{i_2\;(\Varid{a},\Varid{ts}\mathbin{+\!\!+}[\mskip1.5mu \Varid{l},\Varid{r}\mskip1.5mu])}
  \end{enumerate}
...
\textbf{Exemplo}: Para a árvore \ensuremath{t_1 }, a evolução da fila é:
\begin{tabbing}\ttfamily
~\char91{}Node~5~\char46{}\char46{}\char46{}\char93{}~~~~~~~~~~\char45{}\char62{}~5\char44{}~fila~\char61{}~\char91{}Node~3~\char46{}\char46{}\char46{}\char44{}~Node~7~\char46{}\char46{}\char46{}\char93{}\\
\ttfamily ~\char91{}Node~3~\char46{}\char46{}\char46{}\char44{}~\char46{}\char46{}\char46{}\char93{}~~~~~\char45{}\char62{}~3\char44{}~fila~\char61{}~\char91{}Node~7~\char46{}\char46{}\char46{}\char44{}~Node~1~\char46{}\char46{}\char46{}\char44{}~Node~4~\char46{}\char46{}\char46{}\char93{}\\
\ttfamily ~\char91{}Node~7~\char46{}\char46{}\char46{}\char44{}~\char46{}\char46{}\char46{}\char93{}~~~~~\char45{}\char62{}~7\char44{}~fila~\char61{}~\char91{}Node~1~\char46{}\char46{}\char46{}\char44{}~Node~4~\char46{}\char46{}\char46{}\char44{}~Node~6~\char46{}\char46{}\char46{}\char44{}~Node~8~\char46{}\char46{}\char46{}\char93{}\\
\ttfamily ~\char46{}\char46{}\char46{}e~assim~sucessivamente
\end{tabbing}
...
\column{9}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{glevels}\mathrel{=}\alt{\underline{[\mskip1.5mu \mskip1.5mu]}}{\lambda (\Varid{a},(\Varid{ls},\Varid{rs}))\to [\mskip1.5mu \Varid{a}\mskip1.5mu]\mathbin{:}\Varid{zipWithPlus}\;\Varid{ls}\;\Varid{rs}}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}\;\Varid{zipWithPlus}\;[\mskip1.5mu \mskip1.5mu]\;\Varid{ys}\mathrel{=}\Varid{ys}{}\<[E]%
\\
\>[3]{}\hsindent{6}{}\<[9]%
\>[9]{}\Varid{zipWithPlus}\;\Varid{xs}\;[\mskip1.5mu \mskip1.5mu]\mathrel{=}\Varid{xs}{}\<[E]%
\\
\>[3]{}\hsindent{6}{}\<[9]%
\>[9]{}\Varid{zipWithPlus}\;(\Varid{x}\mathbin{:}\Varid{xs})\;(\Varid{y}\mathbin{:}\Varid{ys})\mathrel{=}(\Varid{x}\mathbin{+\!\!+}\Varid{y})\mathbin{:}\Varid{zipWithPlus}\;\Varid{xs}\;\Varid{ys}{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{bft}\;\Varid{t}\mathrel{=}\anaList{\Varid{geneBFT}}\;[\mskip1.5mu \Varid{t}\mskip1.5mu]{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}\;\Varid{geneBFT}\;[\mskip1.5mu \mskip1.5mu]\mathrel{=}i_1\;(){}\<[E]%
\\
\>[3]{}\hsindent{6}{}\<[9]%
\>[9]{}\Varid{geneBFT}\;(\Conid{Empty}\mathbin{:}\Varid{ts})\mathrel{=}\Varid{geneBFT}\;\Varid{ts}{}\<[E]%
\\
\>[3]{}\hsindent{6}{}\<[9]%
\>[9]{}\Varid{geneBFT}\;(\Conid{Node}\;(\Varid{a},(\Varid{l},\Varid{r}))\mathbin{:}\Varid{ts})\mathrel{=}i_2\;(\Varid{a},\Varid{ts}\mathbin{+\!\!+}[\mskip1.5mu \Varid{l},\Varid{r}\mskip1.5mu]){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\textbf{Objetivo}: Calcular o seno hiperbólico de \ensuremath{\Varid{x}} usando \ensuremath{\Varid{n}} termos da sua série de Taylor, de forma eficiente.

\textbf{Série de Taylor do \ensuremath{\Varid{sinh}}}:
\begin{eqnarray*}
...
O catamorfismo de números naturais (\ensuremath{\Varid{worker}}) evolui conforme o seguinte diagrama:
...
&
     \ensuremath{\mathrm{1}\mathbin{+}[\mskip1.5mu \Conid{Double}\mskip1.5mu]}
           \ar[l]^-{\ensuremath{\alt{\underline{\Varid{start}\;\Varid{x}}}{\Varid{loop}\;\Varid{x}}}}
}
\]
...
Para $n=0$, o estado inicial é \ensuremath{\Varid{start}\;\Varid{x}\mathrel{=}[\mskip1.5mu \Varid{x},\Varid{x}\mathbin{\uparrow}\mathrm{3},\mathrm{6},\mathrm{20},\mathrm{22}\mskip1.5mu]}, correspondendo a \ensuremath{[\mskip1.5mu \Varid{s},\Varid{h},\Varid{k},\Varid{j},\Varid{m}\mskip1.5mu]}:
\begin{itemize}
...
\textbf{Iteração do \ensuremath{\Varid{loop}}}: A cada chamada, \ensuremath{\Varid{loop}\;\Varid{x}\;[\mskip1.5mu \Varid{s},\Varid{h},\Varid{k},\Varid{j},\Varid{m}\mskip1.5mu]} calcula:
\begin{itemize}
...
\textbf{Exemplo de execução} para \ensuremath{\Varid{f}\;\mathrm{1.0}\;\mathrm{2}}:
\begin{tabbing}\ttfamily
~Iteracao~0\char58{}~\char91{}1\char46{}0\char44{}~1\char46{}0\char44{}~6\char44{}~20\char44{}~22\char93{}\\
\ttfamily ~Iteracao~1\char58{}~\char91{}1\char46{}0~\char43{}~1\char46{}0\char47{}6\char44{}~1\char46{}0\char44{}~120\char44{}~42\char44{}~30\char93{}~~~~~\char61{}~\char91{}1\char46{}1667\char44{}~1\char46{}0\char44{}~120\char44{}~42\char44{}~30\char93{}\\
\ttfamily ~Iteracao~2\char58{}~\char91{}1\char46{}1667~\char43{}~1\char46{}0\char47{}120\char44{}~1\char46{}0\char44{}~5040\char44{}~72\char44{}~38\char93{}~\char61{}~\char91{}1\char46{}175\char44{}~1\char46{}0\char44{}~5040\char44{}~72\char44{}~38\char93{}
\end{tabbing}

Resultado: \ensuremath{\Varid{head}\;[\mskip1.5mu \mathrm{1.175},\mathbin{...}\mskip1.5mu]\mathrel{=}\mathrm{1.175}}, próximo de $\sinh(1.0) \approx 1.1752$

O \ensuremath{\Varid{wrapper}} (função \ensuremath{\Varid{head}}) extrai o primeiro elemento da lista final, que contém a soma de todos os \ensuremath{\Varid{n}\mathbin{+}\mathrm{1}} termos da série.
...
\column{5}{@{}>{\hspre}l<{\hspost}@{}}%
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{f}\mathbin{::}\Conid{Double}\to \Conid{Int}\to \Conid{Double}{}\<[E]%
\\
\>[B]{}\Varid{f}\;\Varid{x}\;\Varid{n}\mathrel{=}\Varid{wrapper}\;(\Varid{worker}\;\Varid{x}\;\Varid{n}){}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{wrapper}\mathrel{=}\Varid{head}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{worker}\;\Varid{x}\mathrel{=}\cataNat{\alt{\underline{\Varid{start}\;\Varid{x}}}{\Varid{loop}\;\Varid{x}}}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
...
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{loop}\;\Varid{x}\;[\mskip1.5mu \Varid{s},\Varid{h},\Varid{k},\Varid{j},\Varid{m}\mskip1.5mu]\mathrel{=}[\mskip1.5mu \Varid{s}\mathbin{+}\Varid{h}\mathbin{/}\Varid{k},\Varid{x}\mathbin{\uparrow}\mathrm{2}\mathbin{*}\Varid{h},\Varid{k}\mathbin{*}\Varid{j},\Varid{j}\mathbin{+}\Varid{m},\Varid{m}\mathbin{+}\mathrm{8}\mskip1.5mu]{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{sumList}\mathbin{::}\Conid{Num}\;\Varid{a}\Rightarrow [\mskip1.5mu \Varid{a}\mskip1.5mu]\to \Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{sumList}\mathrel{=}\llparenthesis\, \alt{\underline{\mathrm{0}}}{\uncurry{(\mathbin{+})}}\,\rrparenthesis{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{productList}\mathbin{::}\Conid{Num}\;\Varid{a}\Rightarrow [\mskip1.5mu \Varid{a}\mskip1.5mu]\to \Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{productList}\mathrel{=}\llparenthesis\, \alt{\underline{\mathrm{1}}}{\uncurry{(\mathbin{*})}}\,\rrparenthesis{}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{lengthList}\mathbin{::}[\mskip1.5mu \Varid{a}\mskip1.5mu]\to \Conid{Int}{}\<[E]%
\\
\>[B]{}\Varid{lengthList}\mathrel{=}\llparenthesis\, \alt{\underline{\mathrm{0}}}{\succ \comp \p2}\,\rrparenthesis{}\<[E]%
\ColumnHook
\end{hscode}\resethooks

\subsection*{Problema 3}

\textbf{Objetivo}: Implementar uma função \ensuremath{\Varid{fairMerge}} que intercala dois streams (listas infinitas) de forma alternada, usando um anamorfismo.
...
\textbf{Propósito}: A Lei de Fokkinga Dual é a versão para anamorfismos da lei de recursividade mútua. Permite fundir dois anamorfismos mutuamente recursivos num único anamorfismo, usando o combinador \ensuremath{\alt{\cdot }{\cdot }}.

\textbf{Enunciado da lei}:
\begin{eqnarray*}
\ensuremath{\alt{\ana{\Varid{h}}}{\ana{\Varid{k}}}\mathrel{=}\ana{\alt{\Varid{f}}{\Varid{g}}}}
\end{eqnarray*}

sob as condições:
\begin{eqnarray*}
\ensuremath{\begin{lcbr}\Varid{out}\comp \ana{\Varid{h}}\mathrel{=}\fun F \;\alt{\ana{\Varid{h}}}{\ana{\Varid{k}}}\comp \Varid{f}\\\Varid{out}\comp \ana{\Varid{k}}\mathrel{=}\fun F \;\alt{\ana{\Varid{h}}}{\ana{\Varid{k}}}\comp \Varid{g}\end{lcbr}}
\end{eqnarray*}

\textbf{Interpretação}: Dois processos geradores \ensuremath{\Varid{h}} e \ensuremath{\Varid{k}} que se referenciam mutuamente podem ser fundidos num único gerador que escolhe (via \ensuremath{\alt{\cdot }{\cdot }}) qual comportamento seguir em cada passo.

Para Streams, onde \ensuremath{\fun F \;\Conid{X}\mathrel{=}\Conid{A}\times\Conid{X}}, isto significa que podemos ter dois geradores que produzem streams alternando entre si, e fundi-los num único anamorfismo.
...
\begin{eqnarray}
    \ensuremath{\alt{\ana{\Varid{f}}}{\ana{\Varid{g}}}\mathrel{=}\ana{\alt{\Varid{h}}{\Varid{k}}}}
\end{eqnarray}
Assumindo a estrutura de Streams, onde o functor é \ensuremath{\fun F \;\Conid{X}\mathrel{=}\Conid{A}\times\Conid{X}}, a prova desenrola-se da seguinte forma:
...
%
    \ensuremath{\begin{lcbr}\Varid{out}\comp \Varid{f}\mathrel{=}(\Varid{id}\times\alt{\Varid{f}}{\Varid{g}})\comp \Varid{h}\\\Varid{out}\comp \Varid{g}\mathrel{=}(\Varid{id}\times\alt{\Varid{f}}{\Varid{g}})\comp \Varid{k}\end{lcbr}}
%
\just\equiv{ Propriedade Universal do Coproduto (Either) }
%
    \ensuremath{[\mskip1.5mu \Varid{out}\comp \Varid{f},\Varid{out}\comp \Varid{g}\mskip1.5mu]\mathrel{=}(\Varid{id}\times\alt{\Varid{f}}{\Varid{g}})\comp [\mskip1.5mu \Varid{h},\Varid{k}\mskip1.5mu]}
%
\just\equiv{ Lei da Fusão do Either: [f . i, g . j] = f + g }
%
    \ensuremath{\Varid{out}\comp [\mskip1.5mu \Varid{f},\Varid{g}\mskip1.5mu]\mathrel{=}(\Varid{id}\times\alt{\Varid{f}}{\Varid{g}})\comp [\mskip1.5mu \Varid{h},\Varid{k}\mskip1.5mu]}
%
\just\equiv{ Definição de Anamorfismo (Propriedade Universal) }
%
    \ensuremath{[\mskip1.5mu \Varid{f},\Varid{g}\mskip1.5mu]\mathrel{=}\ana{[\mskip1.5mu \Varid{h},\Varid{k}\mskip1.5mu]}}
\qed
\end{eqnarray*}

Esta derivação demonstra que o par de processos independentes \ensuremath{\Varid{f}} e \ensuremath{\Varid{g}} pode ser fundido num único anamorfismo cujo gene é a escolha (\ensuremath{\alt{\cdot }{\cdot }}) entre os dois comportamentos originais. No caso do \ensuremath{\Varid{fairMerge}}, isto permite que o estado alterne entre consumir a "Estrada 1" ou a "Estrada 2".


\subsubsection*{Diagrama do Anamorfismo}

\textbf{Estratégia para \ensuremath{\Varid{fairMerge}}}: 

Em vez de definir \ensuremath{\Varid{fairMerge}} por recursividade mútua (como \ensuremath{\Varid{h}} e \ensuremath{\Varid{k}} no código original), aplicamos a Lei de Fokkinga Dual para obter um único anamorfismo.

O \ensuremath{\Varid{fairMerge}} alterna entre duas streams. O estado do anamorfismo é o par de streams \ensuremath{(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})} e, em cada passo, o sistema:
\begin{enumerate}
...
\\
    \ensuremath{(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})}
           \ar[u]^-{\ensuremath{\Varid{fairMerge}}}
           \ar[r]_-{\ensuremath{\Varid{gene}}}
&
    \ensuremath{\Varid{a}\times(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})}
           \ar[u]_-{\ensuremath{\Varid{id}\times\Varid{fairMerge}}}
}
\]


\textbf{Análise do Gene:}
O gene do \ensuremath{\Varid{fairMerge}} recebe o estado \ensuremath{(\Varid{s1},\Varid{s2})} (par de streams) e:
\begin{enumerate}
  \item Usa \ensuremath{\mathsf{out}\;\Varid{s1}} para decompor a primeira stream em cabeça \ensuremath{\Varid{h}} e cauda \ensuremath{\Varid{t}}
  \item Retorna \ensuremath{(\Varid{h},(\Varid{s2},\Varid{t}))}, onde:
    \begin{itemize}
      \item \ensuremath{\Varid{h}} é o próximo elemento do stream resultante
      \item \ensuremath{(\Varid{s2},\Varid{t})} é o novo estado, com as streams trocadas
    \end{itemize}
\end{enumerate}

\textbf{Porque funciona}: A inversão de \ensuremath{\Varid{s2}} com \ensuremath{\Varid{t}} no novo estado \ensuremath{(\Varid{s2},\Varid{t})} garante que:
\begin{itemize}
  \item Na iteração atual, consumimos de \ensuremath{\Varid{s1}} ("Estrada 1")
  \item Na próxima iteração, o estado é \ensuremath{(\Varid{s2},\Varid{t})}, logo consumiremos de \ensuremath{\Varid{s2}} ("Estrada 2")
  \item Na iteração seguinte, o estado será \ensuremath{(\Varid{t},\Varid{s2'})}, voltando a consumir da original \ensuremath{\Varid{s1}}
\end{itemize}

Esta troca sistemática produz a alternância perfeita: $s1_0, s2_0, s1_1, s2_1, s1_2, s2_2, \ldots$

\textbf{Nota sobre \ensuremath{\Varid{fairMergeM}}}: Esta variante usa um parâmetro adicional \ensuremath{\Varid{p}} (probabilidade) para decidir de qual stream consumir, permitindo modelar situações onde a alternância não é perfeita (e.g., tráfego com diferentes prioridades).
...
\column{E}{@{}>{\hspre}l<{\hspost}@{}}%
\>[B]{}\Varid{fairMerge}\mathbin{::}(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})\to \Conid{Stream}\;\Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{fairMerge}\mathrel{=}\lanabracket\,\Varid{gene}\,\ranabracket{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{gene}\;(\Varid{s1},\Varid{s2})\mathrel{=}\mathbf{let}\;(\Varid{h},\Varid{t})\mathrel{=}\mathsf{out}\;\Varid{s1}{}\<[E]%
\\
\>[5]{}\hsindent{16}{}\<[21]%
\>[21]{}\mathbf{in}\;(\Varid{h},(\Varid{s2},\Varid{t})){}\<[E]%
\\[\blanklineskip]%
\>[B]{}\Varid{fairMergeM}\mathbin{::}(\Conid{Double},(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a}))\to \Conid{Stream}\;\Varid{a}{}\<[E]%
\\
\>[B]{}\Varid{fairMergeM}\mathrel{=}\lanabracket\,\Varid{geneM}\,\ranabracket{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{geneM}\;(\Varid{p},(\Varid{s1},\Varid{s2}))\mathrel{=}{}\<[E]%
\\
\>[5]{}\hsindent{2}{}\<[7]%
\>[7]{}\mathbf{let}\;\Varid{r}\mathrel{=}\mathrm{0.5}{}\<[E]%
\\
\>[5]{}\hsindent{2}{}\<[7]%
\>[7]{}\mathbf{in}\;\mathbf{if}\;\Varid{p}\mathbin{>}\Varid{r}{}\<[E]%
\\
\>[7]{}\hsindent{3}{}\<[10]%
\>[10]{}\mathbf{then}\;\mathbf{let}\;(\Varid{h},\Varid{t})\mathrel{=}\mathsf{out}\;\Varid{s1}\;\mathbf{in}\;(\Varid{h},(\Varid{p},(\Varid{s2},\Varid{t}))){}\<[E]%
\\
\>[7]{}\hsindent{3}{}\<[10]%
\>[10]{}\mathbf{else}\;\mathbf{let}\;(\Varid{h},\Varid{t})\mathrel{=}\mathsf{out}\;\Varid{s2}\;\mathbf{in}\;(\Varid{h},(\Varid{p},(\Varid{s1},\Varid{t}))){}\<[E]%
\ColumnHook
\end{hscode}\resethooks
...
\textbf{Modelo}: Usamos um catamorfismo probabilístico \ensuremath{\Varid{pcataList}\;\Varid{gene}} que, em vez de devolver um resultado determinístico, devolve uma distribuição de probabilidades sobre possíveis resultados.

\textbf{Tipo}: \ensuremath{\Varid{transmitir}\mathbin{::}[\mskip1.5mu \Conid{String}\mskip1.5mu]\to \fun{Dist}\;[\mskip1.5mu \Conid{String}\mskip1.5mu]}

O gene processa a mensagem (lista de palavras) recursivamente:

\begin{itemize}
\item \textbf{Caso base} (\ensuremath{i_1\;()}, lista vazia): 
  
  Chegámos ao fim da mensagem. O aparelho tenta enviar \ensuremath{\text{\ttfamily \char34 stop\char34}}:
  \begin{itemize}
    \item Com probabilidade 90\%: sucesso → resultado é \ensuremath{[\mskip1.5mu \text{\ttfamily \char34 stop\char34}\mskip1.5mu]}
    \item Com probabilidade 10\%: falha → resultado é \ensuremath{[\mskip1.5mu \mskip1.5mu]} (sem stop)
  \end{itemize}
  
  Código: \ensuremath{\Conid{D}\;[\mskip1.5mu ([\mskip1.5mu \text{\ttfamily \char34 stop\char34}\mskip1.5mu],\mathrm{0.9}),([\mskip1.5mu \mskip1.5mu],\mathrm{0.1})\mskip1.5mu]}

\item \textbf{Caso recursivo} (\ensuremath{i_2\;(\Varid{w},\Varid{d})}): 
  
  Recebemos a palavra atual \ensuremath{\Varid{w}} e a distribuição \ensuremath{\Varid{d}} de possíveis continuações (resultado do processamento do resto da lista).
  
  Para cada possível continuação \ensuremath{\Varid{msg}} em \ensuremath{\Varid{d}}, a palavra \ensuremath{\Varid{w}}:
  \begin{itemize}
    \item Com probabilidade 95\%: é transmitida → resultado é \ensuremath{\Varid{w}\mathbin{:}\Varid{msg}}
    \item Com probabilidade 5\%: perde-se → resultado é \ensuremath{\Varid{msg}} (sem \ensuremath{\Varid{w}})
  \end{itemize}
  
  Usamos notação monádica: \ensuremath{\mathbf{do}\;\{\mskip1.5mu \Varid{msg}\leftarrow \Varid{d};\Conid{D}\;[\mskip1.5mu (\Varid{w}\mathbin{:}\Varid{msg},\mathrm{0.95}),(\Varid{msg},\mathrm{0.05})\mskip1.5mu]\mskip1.5mu\}}
  
  Isto combina as probabilidades: se \ensuremath{\Varid{msg}} tem probabilidade \ensuremath{\Varid{p}} em \ensuremath{\Varid{d}}, então:
  \begin{itemize}
    \item \ensuremath{\Varid{w}\mathbin{:}\Varid{msg}} terá probabilidade $p \times 0.95$
    \item \ensuremath{\Varid{msg}} (sem \ensuremath{\Varid{w}}) terá probabilidade $p \times 0.05$
  \end{itemize}
\end{itemize}
...
\xymatrix{
    \ensuremath{[\mskip1.5mu \Conid{String}\mskip1.5mu]}
           ...
           \ar[d]_-{\ensuremath{\Varid{transmitir}}}
&
    \ensuremath{\mathrm{1}\mathbin{+}\Conid{String}\times[\mskip1.5mu \Conid{String}\mskip1.5mu]}
           \ar[d]^{\ensuremath{\Varid{id}\mathbin{+}\Varid{id}\times\Varid{transmitir}}}
\\
...
Para a mensagem \ensuremath{\Varid{words}\;\text{\ttfamily \char34 Vamos~atacar~hoje\char34}\mathrel{=}[\mskip1.5mu \text{\ttfamily \char34 Vamos\char34},\text{\ttfamily \char34 atacar\char34},\text{\ttfamily \char34 hoje\char34}\mskip1.5mu]}:

\begin{itemize}
\item \textbf{Transmissão perfeita}: Manter as 3 palavras ($0.95^3 = 0.857375$) e enviar \ensuremath{\text{\ttfamily \char34 stop\char34}} ($0.9$):
\[P(\text{perfeita}) = 0.857375 \times 0.9 = 0.7716 \text{ (77.16\%)}\]

\item \textbf{Perder "atacar"}: Manter "Vamos" ($0.95$), perder "atacar" ($0.05$), manter "hoje" ($0.95$), enviar \ensuremath{\text{\ttfamily \char34 stop\char34}} ($0.9$):
\[P(\text{perder "atacar"}) = 0.95 \times 0.05 \times 0.95 \times 0.9 = 0.0406 \text{ (4.06\%)}\]

\item \textbf{Todas as palavras mas sem "stop"}: Manter as 3 palavras ($0.857375$) e falhar \ensuremath{\text{\ttfamily \char34 stop\char34}} ($0.1$):
...
\\
\>[B]{}\Varid{gene}\mathrel{=}\alt{\Varid{pzero}}{\Varid{padd}}{}\<[E]%
\\
\>[B]{}\hsindent{3}{}\<[3]%
\>[3]{}\mathbf{where}{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{pzero}\;()\mathrel{=}\Conid{D}\;[\mskip1.5mu ([\mskip1.5mu \text{\ttfamily \char34 stop\char34}\mskip1.5mu],\mathrm{0.9}),([\mskip1.5mu \mskip1.5mu],\mathrm{0.1})\mskip1.5mu]{}\<[E]%
\\
\>[3]{}\hsindent{2}{}\<[5]%
\>[5]{}\Varid{padd}\;(\Varid{w},\Varid{d})\mathrel{=}\mathbf{do}\;\{\mskip1.5mu \Varid{msg}\leftarrow \Varid{d};\Conid{D}\;[\mskip1.5mu (\Varid{w}\mathbin{:}\Varid{msg},\mathrm{0.95}),(\Varid{msg},\mathrm{0.05})\mskip1.5mu]\mskip1.5mu\}{}\<[E]%
\ColumnHook
\end{hscode}\resethooks
