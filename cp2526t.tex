...
A função \ensuremath{\Varid{levels}} visa agrupar os elementos da árvore numa lista de listas, 
onde cada sublista contém os elementos de um nível. Para tal, definimos o catamorfismo \ensuremath{\Varid{levels}\mathrel{=}\llparenthesis\, \Varid{glevels}\,\rrparenthesis}.

O gene \ensuremath{\Varid{glevels}} processa os dois casos do tipo \ensuremath{\Conid{BTree}}: 

\begin{itemize} 

\item No caso \ensuremath{\Conid{Empty}}, retorna uma lista vazia \ensuremath{[\mskip1.5mu \mskip1.5mu]}. 
\item No caso \ensuremath{\Conid{Node}\;(\Varid{a},(\Varid{l},\Varid{r}))}, onde \ensuremath{\Varid{l}} e \ensuremath{\Varid{r}} são as listas de listas já calculadas para as subárvores, 
o gene coloca \ensuremath{\Varid{a}} numa lista unitária (nível 0) e utiliza a função \ensuremath{\Varid{zipWithPlus}} para fundir os níveis restantes das duas subárvores. 
...
Para a função \ensuremath{\Varid{bft}}, utilizamos a estratégia sugerida por Okasaki, baseada numa fila (\ensuremath{\Varid{queue}}).
 Esta função é um anamorfismo de listas onde o estado é uma lista de árvores \ensuremath{[\mskip1.5mu \Conid{BTree}\;\Varid{a}\mskip1.5mu]}.

O gene \ensuremath{\Varid{geneBFT}} funciona da seguinte forma: 
\begin{itemize} 

\item Se a fila estiver vazia (\ensuremath{[\mskip1.5mu \mskip1.5mu]}), a travessia termina. 
\item Se o primeiro elemento for \ensuremath{\Conid{Empty}}, ignoramos e continuamos com o resto da fila. 
\item Se for um \ensuremath{\Conid{Node}\;(\Varid{a},(\Varid{l},\Varid{r}))}, extraímos o valor \ensuremath{\Varid{a}} e adicionamos as subárvores \ensuremath{\Varid{l}} e \ensuremath{\Varid{r}} ao 
...
A função \ensuremath{\Varid{f}\;\Varid{x}\;\Varid{n}} implementa o cálculo do seno hiperbólico através da sua série de Taylor:
\begin{eqnarray*}
...
Para $n=0$, o estado inicial é \ensuremath{\Varid{start}\;\Varid{x}\mathrel{=}[\mskip1.5mu \Varid{x},\Varid{x}\mathbin{\uparrow}\mathrm{3},\mathrm{6},\mathrm{20},\mathrm{22}\mskip1.5mu]}. A lógica dos acumuladores é a seguinte:
\begin{itemize}
...
O \ensuremath{\Varid{wrapper}} (função \ensuremath{\Varid{head}}) extrai o primeiro elemento da lista, que é a soma acumulada final \ensuremath{\Varid{s}} após \ensuremath{\Varid{n}} iterações.
...
Para Streams (listas infinitas), o anamorfismo é definido pelo gene \ensuremath{\Varid{g}\mathbin{:}\Conid{S}\to \Conid{A}\times\Conid{S}}. A Lei de Fokkinga Dual permite-nos fundir dois anamorfismos que partilham a mesma estrutura de saída, simplificando a composição de sistemas dinâmicos. A sua derivação baseia-se na propriedade de que o produto de dois anamorfismos é equivalente a um anamorfismo cujo gene é o produto dos genes originais, garantindo a comutatividade do diagrama.
...
O \ensuremath{\Varid{fairMerge}} alterna entre duas streams. O estado do anamorfismo é o par de streams \ensuremath{(\Conid{Stream}\;\Varid{a},\Conid{Stream}\;\Varid{a})} e, em cada passo, o sistema retira um elemento de uma e passa o controlo para a outra (trocando a ordem no par).
...
O gene do \ensuremath{\Varid{fairMerge}} recebe duas streams \ensuremath{(\Varid{s1},\Varid{s2})}. Ele utiliza o \ensuremath{\mathsf{out}} na primeira para obter a cabeça \ensuremath{\Varid{h}} e a cauda \ensuremath{\Varid{t}}, e devolve o par \ensuremath{(\Varid{h},(\Varid{s2},\Varid{t}))}. Note-se a inversão de \ensuremath{\Varid{s2}} com \ensuremath{\Varid{t}}, que é o que garante a alternância entre as estradas (Braga e Porto) em cada iteração.
...
O problema modela um aparelho de telegrafia avariado usando um catamorfismo probabilístico \ensuremath{\Varid{pcataList}\;\Varid{gene}}. O gene deve processar dois casos:

\begin{itemize}
\item \textbf{Caso base} (lista vazia): O aparelho envia o código \ensuremath{\text{\ttfamily \char34 stop\char34}} com 90\% de probabilidade, ou não envia nada (lista vazia) com 10\% de probabilidade (falha).

\item \textbf{Caso recursivo} (palavra \ensuremath{\Varid{w}} e resto \ensuremath{\Varid{ms}}): A palavra é mantida com 95\% de probabilidade (\ensuremath{\Varid{w}\mathbin{:}\Varid{msg}}), ou perde-se com 5\% de probabilidade (\ensuremath{\Varid{msg}}).
\end{itemize}
...
\xymatrix@C=3cm@R=1.5cm{
    \ensuremath{[\mskip1.5mu \Conid{String}\mskip1.5mu]} 
           ...
           \ar[r]^-{\ensuremath{\Varid{outList}}} 
& 
    \ensuremath{\mathrm{1}\mathbin{+}\Conid{String}\lambda \Varid{times}\;[\mskip1.5mu \Conid{String}\mskip1.5mu]} 
           \ar[d]^{\ensuremath{\Varid{id}\mathbin{+}\Varid{id}\lambda \Varid{times}\;\Varid{transmitir}}} 
\\
    \ensuremath{\fun{Dist}\;[\mskip1.5mu \Conid{String}\mskip1.5mu]} 
& 
    \ensuremath{\mathrm{1}\mathbin{+}\Conid{String}\lambda \Varid{times}\;\fun{Dist}\;[\mskip1.5mu \Conid{String}\mskip1.5mu]} 
           \ar[l]^-{\ensuremath{\Varid{gene}}}
}
\end{eqnarray*}
